...
class CosmicFishPlotter():
    ...
    def figure_1D( self, subplot, param, names, **kwargs ):
        ...
        rangetofit = self.setting_setter('range_dim', **kwargs)
            ...
            for i,confl in enumerate(confidence_level):
                ...
                for i,name in enumerate(names):
                    subplot.fill_between( data[param][name][0], 0.0, data[param][name][1],
                                          ...
                                          alpha     = filling_alpha )
        # get the data:
        data = self.plot_fishers.compute_gaussian( params=param, confidence_level=0.9999999980268247, names=names_temp, num_points=3*num_points, normalized=normalized, nice_bounds=nice )
        # do the plotting:
        for i,name in enumerate(names):
        ...
        x_limits = self.plot_fishers.compute_plot_range(params=param, confidence_level=max(confidence_level), range_factors=range_factors, names=names_temp, nice=nice, dimensions=rangetofit)[param]
    ...
    def figure_2D( self, subplot, param1, param2, names, **kwargs ):
        ...
        # set the plot range:
        ranges = self.plot_fishers.compute_plot_range(params=[param1,param2], confidence_level=max(confidence_level), range_factors=range_factors, names=names_temp, nice=nice, dimensions=rangetofit)
        #print(ranges[param1])
        #print(ranges[param2])
        rang1 = ranges[param1]
        rang2 = ranges[param2]
    ...
    def set_triplot_dimensions( self, num_col, num_rows, **kwargs ):
        ...
        x_size_in               = self.setting_setter('figure_width', **kwargs)
        y_size_in               = self.setting_setter('figure_height', **kwargs)
        triplot_subspace_cm     = self.setting_setter('triplot_subspace_cm', **kwargs)
        subplot_x_size          = self.setting_setter('subplot_x_size', **kwargs)
        subplot_y_size          = self.setting_setter('subplot_y_size', **kwargs)
        use_fixed_figure_width  = self.setting_setter('use_fixed_figure_width', **kwargs)
        use_fixed_figure_height = self.setting_setter('use_fixed_figure_height', **kwargs)
        ...
        tri_subspace = triplot_subspace_cm # default: 0.5 cm
        # compute x_size and y_size:
    ...
    def set_legend( self, names=None, **kwargs ):
        ...
        names_legend = [ u'$\\mathrm{'+str(i).replace(" ", "\ ")+'}$' for i in names_temp]
